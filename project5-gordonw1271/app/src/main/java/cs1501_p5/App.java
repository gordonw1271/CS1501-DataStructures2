...
public class App {
    public static void main(String[] args) {
        ...
        try {
            // Load bitmap image
            BufferedImage image = ImageIO.read(new File("build/resources/main/lbj.bmp"));

            // Create pixel matrix
            Pixel[][] pixelMatrix = convertBitmapToPixelMatrix(image);

            SquaredEuclideanMetric m = new SquaredEuclideanMetric();
            CircularHueMetric cm = new CircularHueMetric();

            BucketingMapGenerator x = new BucketingMapGenerator();
            ClusteringMapGenerator y = new ClusteringMapGenerator(cm);

            ColorQuantizer cq = new ColorQuantizer(pixelMatrix, y);
            cq.quantizeToBMP("output.bmp", 5);
